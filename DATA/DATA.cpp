...
ostream& Patron::printPatron(ostream& out) const
{
	...
	if (!books.empty())
	{
		out << "has the following books:" << endl;
...
void returnBook()
{
	...
	{
		author.name = getString("Enter author's name: ");
		authorRef = find(catalog[author.name[0]].end(), catalog[author.name[0]].end(), author);
		if (authorRef == catalog[author.name[0]].end())
			cout << "Author's name misspelled\n";
		else
			break;
	}
	while (true)
